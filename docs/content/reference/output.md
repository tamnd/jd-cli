...
describes what users get. Pick a format with `-o`, or let jd choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
jd <command> -o table   # aligned columns for reading
jd <command> -o jsonl   # one JSON object per line, for piping
jd <command> -o json    # a single JSON array
jd <command> -o csv     # spreadsheet friendly
jd <command> -o tsv     # tab-separated
jd <command> -o url     # just the URL column
jd <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
jd <command> --fields id,title,url
```
...
```bash
jd <command>            # a table, because this is a terminal
jd <command> | wc -l    # JSONL, because this is a pipe
```
